...
if(isset($_POST['update_product'])){
   ...
   if (isset($_FILES['product_image']) && is_uploaded_file($_FILES['product_image']['tmp_name'])) {
       ...
       $update_data = "UPDATE inventory SET products_id='$product_products_id', products_name='$product_name', image='$imgContent', total_units='$product_total_units', reserved_units='$product_reserved_units' WHERE id = '$product_id'";
   } else {
       // If no image uploaded, update SQL query without image data
       $update_data = "UPDATE inventory SET products_id='$product_products_id', products_name='$product_name', total_units='$product_total_units', reserved_units='$product_reserved_units' WHERE id = '$product_id'";
   }
